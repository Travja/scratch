...
export const makeOutlookInvite = (office: boolean, date: { start: Date, end: Date }, summary: string, description: string, location?: string) => {
  let str = `https://outlook.${office ? 'office' : 'live'}.com/calendar/0/deeplink/compose?allday=false&body=${encodeURIComponent(description)}&enddt=${encodeURIComponent(standardizeDate(date.end))}&path=/calendar/action/compose&rru=addevent&startdt=${encodeURIComponent(standardizeDate(date.start))}&subject=${encodeURIComponent(summary)}`;
