...
{#if activeIndex > -1}
  <div class='modal'
    ...
    <div transition:fly={{ y: 100, duration: 300 }}
         class='modal-content'>
      <img src={imageData[activeIndex].download_url} alt={imageData[activeIndex].author} />
    </div>

    <div class='previous'
         on:click|stopPropagation={() => activeIndex = (activeIndex - 1 + imageData.length) % imageData.length}
         on:keypress={(e) => {
            if (e.key === "Enter") {
    ...
    <div class='close-button' on:click={() => activeIndex = -1}
         on:keypress={(e) => {
           if (e.key === "Enter") {
             activeIndex = -1;
           }
