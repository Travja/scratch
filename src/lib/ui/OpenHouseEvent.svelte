<script lang="ts">
  ...
  import { makeIcsFileOhio, makeOutlookInvite } from "../../api/calendar";
  ...
  const date = {
    start: new Date("2023/09/02 22:30:00 UTC"),
    end: new Date("2023/09/03 00:30:00 UTC")
  };
  const title = "Dorothy + Travis' Ohio Open House";
  const description = `*Note the time is likely reflecting 6:30-8:30 in your current time zone. The event is 6:30-8:30 EST.\n\nWe're getting married in Utah, but we want to celebrate with our friends and family in Ohio too!\n\nGift Info\nRegistered on Amazon: ${amazonLink}\nVenmo: @${venmoUsername}\nWebsite: https://stellarmelodies.com`;
  const address = "4610 N Bend Rd, Cincinnati, OH, United States";

  let icsFile: string = makeIcsFileOhio(
    date,
    title,
...
>
  ...
  {#if shown}
    ...
    >
      <a
        href="https://calendar.google.com/calendar/event?action=TEMPLATE&tmeid=MDlwM3M1ajJicmhwZmRyOGN0ZTluMGltaGQgYjBjNGNkMzI4NjcxNzFkOTdhZjc5NzZhMzBhMTBhYWE2NDIxMTNmOWM4M2MyNmU5ZjBhMzM3YTU5NTEzM2UyOEBn&tmsrc=b0c4cd32867171d97af7976a30a10aaa642113f9c83c26e9f0a337a595133e28%40group.calendar.google.com"
        target="_blank"
        class="invite"
      >
