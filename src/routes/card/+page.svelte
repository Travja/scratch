  ...
  const trackMouse = (e: MouseEvent | TouchEvent) => {
    ...
    if (e instanceof MouseEvent) {
      ...
      if (diffX > 200 || diffX < -200) {
        ...
        startX = -1;
        flip(e, flipDirection as 'left' | 'right' | undefined);
        return false;
      }
    }
    ...
    card.style.transform = `translateY(-50%) rotateX(${-percentY * 10}deg) rotateY(${
      percentX * 10
    }deg)`;
    if (card === frontCard) {
      rotation = 0;
...
>
  <div
    ...
    class:shadow
  >
    <div class="info">Here is some fancy little info</div>
  </div>
  <div
    class="card"
    id="back"
    ...
    class:shadow
  >
    <div class="info">Other fancy info</div>
  </div>
</div>
